...
pub fn solve(start: Map, solver: Solver) -> Result<(), String>
{
	...
	let mut end_node = loop
	{
		...
		{
			list.append(&mut expand_node(node, iter, limit, &mut closed_set, &solver, &mut debug));
		...
		// if solver.flag.debug { node = debug.parent(node, solver.size, list.len() + 1, closed_set.len()) }
		// list.push(node);
